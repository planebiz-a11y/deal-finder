...
function extractPrice(text: string): number {
  ...
  const numbers = matches
    ...
    .filter((n) => n > 200)           // ignore noise like $5
    .filter((n) => !(n >= 1900 && n <= 2099)) // ignore years
...
function extractHours(text: string): number | null {
  // matches: "1200 hours", "800 hrs", "1.2k hours"
...
function extractMileage(text: string): number | null {
  // matches: "12,000 miles", "45k miles", "12000 mi"
...
function estimateResaleValue(
  ...
  // ── Year adjustment (neutral default = 5 yrs old if unknown)
  ...
  // ── Hours adjustment (neutral default = no change if unknown)
  if (hours !== null) {
  ...
  // ── Mileage adjustment (neutral default = no change if unknown)
  if (mileage !== null) {
...
function estimateCosts(
  ...
  const text = `${title} ${description}`.toLowerCase()
  let cost = 400 // base: fuel, time, listing fees
  ...
  // High hours = more wear even if running fine
...
function getRiskFlags(
  ...
  // Limited info — don't penalize, just surface it
...
function scoreDeal(
  ...
  // Risk deductions — "Limited info" costs less than real risk flags
  const limitedInfoFlag = riskFlags.includes("Limited info — ask seller")
  const hardRiskCount = riskFlags.filter(
    (f) => f !== "Limited info — ask seller" && f !== "Older unit"
  ).length

  score -= hardRiskCount * 0.75
  if (limitedInfoFlag) score -= 0.25 // tiny nudge, not a penalty
  ...
  try {
    ...
    const analyzedDeals = listings
      .map((listing) => {
        ...
        const estimatedResaleValue = estimateResaleValue(
          title, price, condition, year, hours, mileage
        )
        ...
        const riskFlags = getRiskFlags(
          condition, title, description, year, hours, mileage, price
        )
      ...
      .filter((deal) => deal.price > 0) // drop listings with no extractable price
